...
export function patch(n1, n2, container, anchor) {
  ...
  if (shapeFlag & ShapeFlags.COMPONENT) processComponent(n1, n2, container, anchor);else if (shapeFlag & ShapeFlags.TEXT) processText(n1, n2, container, anchor);else if (shapeFlag & ShapeFlags.FRAGMENT) processFragment(n1, n2, container, anchor);else if (shapeFlag & ShapeFlags.ELEMENT) processElement(n1, n2, container, anchor);
}
...
function unmount(vnode) {
  ...
  if (shapeFlag & ShapeFlags.COMPONENT) unmountComponent(vnode);else if (shapeFlag & ShapeFlags.FRAGMENT) unmountFragment(vnode);else {
...
function patchKeyedChildren(c1, c2, container, anchor) {
  ...
  for (let i = 0; i < c2.length; i++) {
    const next = c2[i]; //第一个插入原来的第一个之前，之后依次插入第一个后面
